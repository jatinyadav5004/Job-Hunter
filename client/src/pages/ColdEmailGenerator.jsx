...
export default function ColdEmailGenerator() {
  ...
  const handleOpenOutlook = () => {
    ...
    const sub = encodeURIComponent(subject || '');
    const bod = encodeURIComponent(emailBody || '');
    window.open(`https://outlook.live.com/mail/0/deeplink/compose?to=${to}&subject=${sub}&body=${bod}`, '_blank');
  };
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      const errMsg = err.response?.data?.message || 'Platform SMTP is not configured or failed to dispatch.';
      setStatusMessage({
        type: 'error',
        msg: `${errMsg} You can directly open and send from your Mail App or Gmail below!`,
        allowMailApp: true,
      });
    } finally {
      setSending(false);
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6">
      ...
      {statusMessage.msg && (
        <div
          className={`p-4 rounded-2xl border text-xs font-semibold flex flex-col sm:flex-row sm:items-center justify-between gap-3 animate-in fade-in ${
        ...
        >
          ...
          {statusMessage.allowMailApp && (
            <div className="flex items-center gap-2 shrink-0">
              <button
                type="button"
                onClick={handleOpenMailApp}
                className="px-3 py-1.5 bg-teal-600 hover:bg-teal-700 text-white rounded-lg text-xs font-bold flex items-center gap-1 shadow-xs"
              >
              ...
              <button
                ...
                className="px-3 py-1.5 bg-white border border-slate-200 text-slate-700 hover:bg-slate-50 rounded-lg text-xs font-bold flex items-center gap-1"
              >
                <span>Gmail Web</span>
              </button>
            </div>
              ...
              <div className="flex flex-wrap items-center justify-between gap-2.5">
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  <button
                    type="button"
                    onClick={handleOpenMailApp}
                    className="px-4 py-2 rounded-xl bg-teal-50 border border-teal-200 text-teal-800 hover:bg-teal-100 font-bold text-xs flex items-center gap-1.5 transition-colors shadow-2xs"
                    title="Opens your system default email client (Outlook, Apple Mail, etc.)"
                  >
                  ...
                  <button
                    type="button"
                    onClick={handleOpenGmail}
                    className="px-3.5 py-2 rounded-xl bg-slate-50 border border-slate-200 text-slate-700 hover:bg-slate-100 font-bold text-xs flex items-center gap-1.5 transition-colors"
                  ...
                  <button
                    type="button"
                    onClick={handleCopy}
                    className="px-3.5 py-2 rounded-xl border border-slate-200 bg-white hover:bg-slate-50 text-slate-700 font-bold text-xs flex items-center gap-1.5 transition-colors"
                  >
                ...
                <button
                  ...
                  className="px-5 py-2 rounded-xl bg-teal-600 hover:bg-teal-700 text-white font-bold text-xs flex items-center justify-center gap-2 shadow-md shadow-teal-600/20 transition-all disabled:opacity-50"
                  title="Send through JobHunter SMTP delivery engine"
                >
